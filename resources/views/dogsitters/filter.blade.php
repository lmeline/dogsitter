        ...
        <form method="GET" action="{{ route('dogsitters.index') }}" class="mb-6">
            <div class="flex items-center space-x-4">
                ...
                <div class="flex-1">
                    ...
                    <input type="number" name="tarif_min" id="tarif_min" value="{{ request('tarif_min') }}"
                        class="w-full border border-gray-300 rounded-lg p-3" placeholder="Ex: 10">
                </div>

                <div class="flex-1">
                    <label for="tarif_max" class="block text-gray-700">Tarif Maximum (€)</label>
                    <input type="number" name="tarif_max" id="tarif_max" value="{{ request('tarif_max') }}"
                        class="w-full border border-gray-300 rounded-lg p-3" placeholder="Ex: 50">
                </div>

                <div class="flex items-center justify-center">
                    <button type="submit"
                        class="bg-gradient-to-r from-yellow-300 to-pink-300 text-white px-6 py-3 rounded-lg hover:from-yellow-400 hover:to-pink-400 transition">
                        Filtrer
                    </button>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            @foreach($dogsitters as $dogsitter)
                <div class="bg-white p-6 rounded-lg shadow-md">
                    ...
                    <a href="{{ route('dogsitters.show', $dogsitter->id) }}" class="mt-4 text-blue-500 hover:underline">Voir
                        le profil</a>
                </div>
            @endforeach
        </div>
